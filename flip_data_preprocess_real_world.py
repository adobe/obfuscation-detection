...
for label_file in LABEL_FILES:
    ...
    for _, row in csv.iterrows():
        ...
        if not ((ps_path.startswith('GithubGist') or ps_path.startswith('PoshCode') or ps_path.startswith('TechNet')) and \
            int(row[1]) == 1):
            continue
...
x = torch.stack(converted_tensors)
y = torch.stack(tensor_labels)

print(x.shape, y.shape)

torch.save({'x': x, 'y': y}, PROCESSED_TENSORS_DIR + 'flip_train_data_real_world.pth')
torch.save(filenames, 'filenames_real_world.pth')
